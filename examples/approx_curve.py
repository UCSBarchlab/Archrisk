...
def get_xy2normed_perf(perf_model, xy2z, reevaluate=False):
    ...
    for coord in xy2perf:
        ...
        xy2normed_p[coord] = (perf_pdesign, risk_pdesign)
        xy2normed_r[coord] = (perf_rdesign, risk_rdesign)
        logging.debug('Coord ({}, {}):\nconv_perf: {} ({})\nperf_pdesign: {} ({})'.format(
...
def prepare_plot(perf_model, xy2z_gt, xy2z_ap):
    xy2gt_p, xy2gt_r = get_xy2normed_perf(perf_model, xy2z_gt)
    xy2ap_p, xy2ap_r = [], []
    for xy2z in xy2z_ap:
        xy2normed_ap_p, xy2normed_ap_r = get_xy2normed_perf(perf_model, xy2z, True)
        xy2ap_p.append(xy2normed_ap_p)
        xy2ap_r.append(xy2normed_ap_r)

    with open(args.pickle, 'w') as f:
        data = (xy2gt_p, xy2gt_r, xy2ap_p, xy2ap_r)
        pickle.dump(data, f)
        f.close()

    return xy2gt_p, xy2gt_r, xy2ap_p, xy2ap_r

def plot_approx_curve(xy2normed_gt_p, xy2normed_gt_r, xy2normed_ap_p, xy2normed_ap_r):
    X, Y = [], []
    ...
    X = sorted(X)
    Y = sorted(Y)
    sigma1, sigma2, sigma3, sigma4, sigma5 = .2, .4, .6, .8, 1.
    perf_ap_p, risk_ap_r = [], []
    for xy2ap_p, xy2ap_r in zip(xy2normed_ap_p, xy2normed_ap_r):
        perf1, perf2, perf3, perf4, perf5 = 0., 0., 0., 0., 0.
        risk1, risk2, risk3, risk4, risk5 = 0., 0., 0., 0., 0.
        count1, count2, count3, count4, count5 = 0, 0, 0, 0, 0
        for x in X:
            for y in Y:
                coord = (x, y)
                gt_perf = xy2normed_gt_p[coord][0]
                gt_risk = xy2normed_gt_r[coord][1]
                if coord not in xy2ap_p or coord not in xy2ap_r:
                    continue
                perf = np.abs(gt_perf - xy2ap_p[coord][0]) / gt_perf
                risk = np.abs(xy2ap_r[coord][1] - gt_risk) / gt_risk if gt_risk else 0
                if x <= sigma1 and y <= sigma1:
                    perf1 += perf
                    risk1 += risk
                    count1 += 1
                if x <= sigma2 and y <= sigma2:
                    perf2 += perf
                    risk2 += risk
                    count2 += 1
                if x <= sigma3 and y <= sigma3:
                    perf3 += perf
                    risk3 += risk
                    count3 += 1
                if x <= sigma4 and y <= sigma4:
                    perf4 += perf
                    risk4 += risk
                    count4 += 1
                if x <= sigma5 and y <= sigma5:
                    perf5 += perf
                    risk5 += risk
                    count5 += 1
                logging.debug('Coord ({}, {}):\nNormlized perf: {}\nNormlized risk: {}'.format(
                    x, y, perf, risk))
        perf_ap_p.append((perf1/count1, perf2/count2, perf3/count3, perf4/count4, perf5/count5))
        risk_ap_r.append((risk1/count1, risk2/count2, risk3/count3, perf4/count4, perf5/count5))

    if not args.fc_file and not args.cpudb_dir:
        title = 'approx_curve_' + str(args.f) + '_' + str(args.c)
    elif not args.fc_file:
        title = 'cpudb_approx_curve_' + str(args.f) + '_' + str(args.c)
    elif not args.cpudb_dir:
        title = 'kde_approx_curve_' + args.fc_file[args.fc_file.rfind('/')+1:args.fc_file.rfind('.')]
    else:
        title = 'cpudb_kde_approx_curve_' + args.fc_file[args.fc_file.rfind('/')+1:args.fc_file.rfind('.')]
    xs = np.log([10000, 1000, 100, 50, 20])
    assert len(xs) == len(perf_ap_p)
    PlotHelper.plot_approx_curve(xs, perf_ap_p, risk_ap_r, title)    
...
def load_from_files():
    assert args.load_path_gt and args.load_path_ap and args.load_path_gt not in args.load_path_ap
    logging.debug('Loading from gt {}'.format(args.load_path_gt))
    with open(args.load_path_gt, 'r') as f:
        xy2z_gt = pickle.load(f)
        f.close()
    xy2z_ap = []
    for path in args.load_path_ap:
        logging.debug('Loading from ap {}'.format(path))
        with open(path, 'r') as f:
            tmp_xy2z_ap = pickle.load(f)
            xy2z_ap.append(tmp_xy2z_ap)
            f.close()
...
def parse_args():
    ...
    parser.add_argument('--load-path-ap', action='store', nargs='+', default=None,
